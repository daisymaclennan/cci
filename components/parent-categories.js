...
const ParentCategories = ({ categories}) => {
  ...
  function findActiveCat(catId){
    if(router.query.cat){
      if(router.query.cat === catId){
        console.log(router.query.cat + "is being compared with" + catId)
        return true
      }
    }
    return false
        ...
        <OrgCategoryHorizontal key={category.category_id}>
            <Link href={`/directory?cat=${category.category_id}`}>
              <a>
                ...
                <svg className={`${findActiveCat(category.category_id) ? "active-cat" : "inactive-cat"} cat-blip`} width="6" height="24" viewBox="0 0 6 24" fill="none" xmlns="http://www.w3.org/2000/svg">
